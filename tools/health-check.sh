    ...
    case "$check_type" in
        ...
        docker)
            if command -v docker &>/dev/null && run_sudo docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q "^${check_target}$"; then
                ...
                version=$(run_sudo docker inspect --format='{{.Config.Image}}' "$check_target" 2>/dev/null | cut -d':' -f2 | cut -d'@' -f1)
            fi
            ;;
    esac
...
main() {
    ...
    if [ "$mode" = "--html" ]; then
    ...
    elif [ "$mode" = "--status" ] || [ "$mode" = "--cli" ]; then
        generate_status_table
    else
        print_header
        check_system_resources
        ...
        echo "=============================================="
        echo "  Tip: Run with --status for a tabular view"
    fi
}
