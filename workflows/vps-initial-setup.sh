...
gather_information() {
    ...
    # Stored in GENERATED_USER_PASSWORD for display at end of setup (never re-shown)
...
create_admin_user() {
    ...
    # Set root password (required for `su -` from user account)
    # Root SSH login remains DISABLED (key-only via authorized_keys + PermitRootLogin no)
    GENERATED_ROOT_PASSWORD=$(generate_system_password 20)
    echo "root:${GENERATED_ROOT_PASSWORD}" | run_sudo chpasswd
    log_success "Root password set (use 'su -' to switch to root, SSH login for root is disabled)"
    ...
    # systemctl --user, Docker rootless, PM2, etc. without manual setup.
    {
        ...
        echo ""
        echo "# Standard paths for utilities and scripts (prioritizing /usr/local/bin for overrides)"
        echo "export PATH=\"\$HOME/.local/bin:\$HOME/bin:/usr/local/bin:\$PATH\""
        echo ""
        echo "# Standard umask for development (files=644, dirs=755)"
        echo "umask 022"
    } | run_sudo tee -a "$USER_HOME/.bashrc" > /dev/null

    log_success "Systemd user environment + umask 022 injected into $USER_HOME/.bashrc"
    # --- END SYSTEMCTL FIX ---
    ...
    if [ -f "/root/.ssh/authorized_keys" ]; then
        ...
        if [ -n "$SSH_KEY_INPUT" ]; then
            # Validate that the input looks like a valid SSH public key
            if echo "$SSH_KEY_INPUT" | ssh-keygen -l -f - &>/dev/null 2>&1; then
                ...
                log_warn "Invalid SSH public key format! Skipping - add your key manually later."
    ...
    # Permisiuni la nivel de grup pentru librariile instalate global si binare
    # Aplicam SetGID doar pe /usr/local (nu pe /opt in totalitate pentru a nu strica aplicatii terțe)
    if getent group "$SUDO_GROUP" &>/dev/null; then
        run_sudo mkdir -p /usr/local/bin /usr/local/lib /usr/local/sbin
        run_sudo chown -R root:"$SUDO_GROUP" /usr/local/bin /usr/local/lib /usr/local/sbin
        run_sudo chmod -R g+rwxs /usr/local/bin /usr/local/lib /usr/local/sbin
        # Pentru /opt cream un director admin propriu in loc sa modificam tot /opt
        run_sudo mkdir -p /opt/admin
        run_sudo chown root:"$SUDO_GROUP" /opt/admin
        run_sudo chmod g+rwxs /opt/admin
        log_success "Group permissions (setgid) applied to /usr/local/* and /opt/admin for group $SUDO_GROUP"
    else
        log_warn "Group $SUDO_GROUP does not exist. Skipping global directory permissions."
    fi
...
# Monitor local binary directories (high-value attack surface)
-w /usr/local/bin -p wa -k local_binaries
-w /usr/local/sbin -p wa -k local_binaries
-w /opt/admin -p wa -k local_binaries
...
configure_auto_updates() {
    ...
    if ! is_debian_based; then
        log_info "Skipping unattended-upgrades setup (not Debian/Ubuntu based)"
        return 0
    fi
...
main() {
    ...
    # Securizează grupul administrativ (doar root poate modifica grupul sudo/wheel)
    log_step "Step 12.1: Securing Admin Group Modifications"
    local ADMIN_GROUP
    if is_debian_based; then ADMIN_GROUP="sudo"; else ADMIN_GROUP="wheel"; fi
    
    run_sudo bash -c "cat > /etc/sudoers.d/01-restrict-admin-group" <<EOF
# Previne modificarea grupului de administratori (${ADMIN_GROUP}) de la nivel de sudo
# Doar "root" autentificat direct (sau via su -) poate executa aceste comenzi.
Defaults!/usr/sbin/usermod, /usr/sbin/gpasswd, /usr/sbin/useradd, /usr/sbin/deluser, /usr/sbin/addgroup !authenticate
Cmnd_Alias RESTRICTED_USER_MOD = /usr/sbin/usermod * ${ADMIN_GROUP}*, /usr/sbin/gpasswd * ${ADMIN_GROUP}*, /usr/sbin/useradd * ${ADMIN_GROUP}*, /usr/sbin/deluser * ${ADMIN_GROUP}*, /usr/sbin/addgroup * ${ADMIN_GROUP}*
ALL ALL=(ALL) !RESTRICTED_USER_MOD
EOF
    run_sudo chmod 0440 /etc/sudoers.d/01-restrict-admin-group
    log_success "Admin group (${ADMIN_GROUP}) modifications restricted to true root only"
    ...
    echo ""
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_warn "  ⚠️  SAVE THESE CREDENTIALS - SHOWN ONLY ONCE"
    log_warn "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    ...
    if [[ "${FORCE_YES:-}" == "1" ]]; then
        final_confirm="YES"
        echo -e "${YELLOW}Restarting SSH automatically (FORCE_YES is set)...${NC}"
    else
